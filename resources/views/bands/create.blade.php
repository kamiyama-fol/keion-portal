    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-4">
                ...
                @else
                    <a href="{{ route('bands.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-400 focus:bg-gray-400 active:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        {{ __('一覧に戻る') }}
                    </a>
                @endif
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('bands.store') }}" class="space-y-6" id="band-form">
                        @csrf
                        ...
                        @else
                            <div>
                                ...
                                <select id="live_id" name="live_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                    <option value="">選択してください</option>
                                    @foreach(App\Models\Live::orderBy('held_date', 'desc')->get() as $liveOption)
                                        <option value="{{ $liveOption->id }}" {{ old('live_id') == $liveOption->id ? 'selected' : '' }}>
